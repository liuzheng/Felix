...
May be called separately from the program to initialize user data
...
def promptForInfo(name, previousInput=""):
    ...
    """
    previous = ""
    # If there was user information set before, display it
    if len(previousInput) > 0:
        previous = " (previous value was %s)" % (previousInput)
...
def setupName(userInfo):
    ...
    """
    userInfo["firstname"] = promptForInfo("First Name",
                                          userInfo.get("firstname", ""))
    userInfo["lastname"] = promptForInfo("Last Name",
...
def setupNickname(userInfo):
    ...
    if promptForInfo("Would you rather I use a nickname? (yes/no)") == "yes":
        userInfo["nickname"] = promptForInfo("Nickname",
                                             userInfo.get("nickname", ""))
    else:
        # Remove any entry for 'nickname' from before
        if "nickname" in userInfo: del userInfo["nickname"]
...
def setupLocation(userInfo):
    ...
    """
    locationState = None # State (inside of US) or country (outside of US)
    if promptForInfo("Are you within the United States? (yes/no)") == "yes":
        locationState = promptForInfo("State Abbreviation (ex. CT)")
    else:
        locationState = promptForInfo("Country (ex. France)")
    locationCity = promptForInfo("City") # City (inside or outside of US)
...
def setupTimezone(userInfo):
    ...
    The TZ* format is required to work with unix and the pytz library
    """
    address = "http://en.wikipedia.org/wiki/List_of_tz_database_time_zones"
    timezone = promptForInfo(("Timezone (select from the TZ* column at %s)" %
                              (address)), userInfo.get("timezone", ""))
...
def setupWundergroundKey(userInfo):
    ...
    print ("To access weather reports, you must obtain a free API key from %s" %
           (address))
...
def setupFacebookKey(userInfo):
    ...
    print ("To access Facebook, you must obtain a free access key from %s" %
           (address))
